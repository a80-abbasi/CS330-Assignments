...
class DataGenerator(object):
    ...
    def sample_batch(self, batch_type, batch_size):
        ...
        for b in range(batch_size):
            folders = np.random.choice(folders, size=self.num_classes)
            batch_labels = np.eye(self.num_classes)
            support_samples = get_images(folders, batch_labels, self.num_samples_per_class, shuffle=False)
            query_samples = get_images(folders, batch_labels, 1, shuffle=True)
            samples = support_samples + query_samples
            batch_labels, batch_paths = map(list, zip(*samples))
            batch_images = [image_file_to_array(path, self.dim_input) for path in batch_paths]
            images.append(batch_images)
            labels.append(batch_labels)
        images = np.array(images).reshape((batch_size, self.num_classes, self.num_samples_per_class+1, -1))
        labels = np.array(labels).reshape((batch_size, self.num_classes, self.num_samples_per_class+1, self.num_classes))
        images = np.transpose(images, (0, 2, 1, 3))
        labels = np.transpose(labels, (0, 2, 1, 3))
